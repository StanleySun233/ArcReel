...
- [ ] Add ArcReel bootstrap status, authorization start, callback handling, and repair start flow under authenticated API routes.
  - Commit: pending
- [ ] Add ArcReel user-scoped persistence for provider credentials, built-in provider configuration, custom providers, custom provider models, provider defaults, and bootstrap completion state.
  - Commit: pending
