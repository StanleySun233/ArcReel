...
def build_arcreel_mcp_server(
    *, project_name: str, projects_root: Path, tenant_id: str | None = None, user_id: str | None = None
) -> Any:
    """Build the per-session in-process MCP server with all ArcReel tools."""
    ctx = ToolContext(project_name=project_name, projects_root=projects_root, tenant_id=tenant_id, user_id=user_id)
