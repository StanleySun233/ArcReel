...
async def _seed(async_session) -> None:
    ...
    await async_session.flush()
    async_session.add(TenantMembership(tenant_id="ten_1", user_id="usr_1", role="admin", created_by_user_id="usr_1"))
    async_session.add(TenantMembership(tenant_id="ten_2", user_id="usr_2", role="admin", created_by_user_id="usr_2"))
...
async def test_project_routes_scope_same_name_by_current_tenant(async_session, tmp_path, monkeypatch) -> None:
    ...
    current = {"user": CurrentUserInfo(id="usr_1", sub="alice", tenant_id="ten_1", tenant_role="admin")}
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
        ...
        tenant_one_project_id = first.json()["id"]

        current["user"] = CurrentUserInfo(id="usr_2", sub="bob", tenant_id="ten_2", tenant_role="admin")
        second = await client.post("/api/v1/projects", json={"name": "demo", "title": "Tenant Two"})
        assert second.status_code == 200, second.text
        tenant_two_project_id = second.json()["id"]

        tenant_two_project = await client.get(f"/api/v1/projects/{tenant_two_project_id}")
        ...
        patch = await client.patch(f"/api/v1/projects/{tenant_one_project_id}", json={"title": "Tenant One Updated"})
        assert patch.status_code == 200, patch.text

        current["user"] = CurrentUserInfo(id="usr_2", sub="bob", tenant_id="ten_2", tenant_role="admin")
        tenant_two_after_patch = await client.get(f"/api/v1/projects/{tenant_two_project_id}")
        assert tenant_two_after_patch.json()["project"]["title"] == "Tenant Two"

    assert (tmp_path / "data" / "_tenants" / "ten_1" / "projects" / tenant_one_project_id / "project.json").exists()
    assert (tmp_path / "data" / "_tenants" / "ten_2" / "projects" / tenant_two_project_id / "project.json").exists()
